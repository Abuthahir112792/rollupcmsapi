...
{   
    ...
    {   
        ...
        return view('admin.user.editorders_list', ['productlist' => $productlist,'orderid'=>$id,'orderstatus'=>$orderstatus->order_status]);
    }
    ...
    public function getuserorderlist($editorderid){
        ...
        $getOrdersuser = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->where('orders.id', Helper::decrypt($editorderid))->select('orders.*', 'users.mobile_number', 'users.address')->first();
    ...
    public function OrderdetailsUpdate(Request $request){
        ...
        if($id != ''){
            ...
            return redirect('admin/orders')->with('message', $msg);
        }
        return redirect('admin/orders')->with('error','Something wrong to update order.');
    }
