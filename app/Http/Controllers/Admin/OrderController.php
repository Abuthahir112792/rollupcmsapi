...
{   
    ...
    {   
        ...
        return view('admin.user.orders_list', ['neworder' => $neworder]);
    }

    public function incomingOrders(){
        $data = [];
        $getOrders = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->leftjoin('branch', 'branch.id', '=', 'orders.branch_id')->where('order_status', '')->orwhere('order_status', 'Pending')->orWhereNull('order_status')->select('orders.*', 'users.mobile_number', 'users.address','branch.branch_name')->get()->toArray();
        
        if(count($getOrders) > 0){
            foreach ($getOrders as $order) {
                $date_time = date('d-m-Y H:i', strtotime($order['created_at']));
                $data[] = [$order['id'],$date_time, $order['order_id'], $order['user_name'], $order['order_description'], $order['order_price'], $order['order_status']."-".$order['id'], $order['mobile_number'], $order['address'], $order['self_pickup'], $order['latitude'], $order['longitude'], $order['branch_name'], Helper::encrypt($order['id'])];
            }
        }
    ...
    public function outForDeliveryOrders(){
        $data = [];
        $getOrders = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->leftjoin('branch', 'branch.id', '=', 'orders.branch_id')->where('order_status', 'Accepted')->where('self_pickup', 'False')->select('orders.*', 'users.mobile_number', 'users.address','branch.branch_name')->get()->toArray();
        
        if(count($getOrders) > 0){
            foreach ($getOrders as $order) {
                $date_time = date('d-m-Y H:i', strtotime($order['created_at']));
                $data[] = [$order['id'],$date_time, $order['order_id'], $order['user_name'], $order['order_description'], $order['order_price'], $order['order_status']."-".$order['id'], $order['mobile_number'], $order['address'], $order['latitude'], $order['longitude'], $order['branch_name'], Helper::encrypt($order['id'])];
            }
        }
    ...
    public function selfPickupOrders(){
        $data = [];
        $getOrders = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->leftjoin('branch', 'branch.id', '=', 'orders.branch_id')->where('order_status', '=' ,'Accepted')->where('self_pickup', '=' ,'True')->select('orders.*', 'users.mobile_number', 'users.address','branch.branch_name')->get()->toArray();
    ...
    public function outForOrderUpdate(Request $request){
        ...
        if($id != ''){
            ...
            return redirect('admin/orders#outfordelivery')->with('message', $msg);
        }
        return redirect('admin/orders#outfordelivery')->with('error','Something wrong to update order.');
    }

    public function neworderAllow($status){
        Orderstatus::select('status')->Where('branch_id','5')->update(["status"=>$status]);
     ...
     public function neworderChecking(){
       ...
       // $to3 = date('Y-m-d H:i',strtotime('-3 minutes',strtotime($from)));
        ...
        $neworderstatus123 = Orderstatus::select('status')->Where('branch_id','5')->first();
...
//echo "Format: $format; " . $date->format('Y/m/d H:i:s') . "\n";
    ...
    public function getBranchDetail($id){
        $orderid = Helper::decrypt($id);
        $orderbranchid = Orders::select('branch_id')->where('id',$orderid)->first();
        $branchdata = Branch::where('id','!=',$orderbranchid->branch_id)->get();
    ...
    public function swiftOrderUpdate(Request $request){
        ...
        $status = "True";
        $ordersbranch = Orders::select('branch_id')->where('id',$id)->first();
        if($id != ''){
            if($request->branch_id == ''){
                $msg = 'Please select any one branch.';
            }
            else{
            Orderstatus::select('status')->Where('branch_id',$ordersbranch->branch_id)->update(["status"=>"False"]);
            ...
            return redirect('admin/orders#incomingorder')->with('message', $msg);
        }
        return redirect('admin/orders#incomingorder')->with('error','Something wrong to update order.');
    }
