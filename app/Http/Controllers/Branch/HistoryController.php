...
use Session;
use Auth;
use App\Exports\BranchOrdersExport;
...
{   
    ...
    {   
        $data= Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->where('order_status', '!=', '')->whereNotNull('order_status')->whereNotIn('order_status', ['Pending','Accepted'])->where('orders.branch_id',Auth::user()->branch_id)->select('orders.*', 'users.mobile_number', 'users.address')->orderBy('orders.id', 'desc')->paginate(10);
         return view('branch.user.history_list', compact('data'));
    }
    ...
    public  function fetch_data(Request $request)
    {
     if($request->ajax())
     {
      ...
      $sort_type = $request->get('sorttype');
           ...
           if($query != '')
            {
                $data = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->where('orders.order_status', '!=', '')->whereNotNull('orders.order_status')->whereNotIn('orders.order_status', ['Pending','Accepted'])->where('orders.branch_id',Auth::user()->branch_id)->where('orders.user_name', 'like', '%'.$query.'%')->orWhere('orders.order_id', 'like', '%'.$query.'%')->orWhere('orders.order_description', 'like', '%'.$query.'%')->orWhere('orders.order_price', 'like', '%'.$query.'%')->orWhere('orders.self_pickup', 'like', '%'.$query.'%')->orWhere('orders.id', 'like', '%'.$query.'%')->orWhere('orders.latitude', 'like', '%'.$query.'%')->orWhere('orders.longitude', 'like', '%'.$query.'%')->select('orders.*', 'users.mobile_number', 'users.address')->orderBy('orders.id', 'desc')->paginate(1)->setpath($page);
                //$data->appends(array('q'=>$request->get('query')));
            }
            else{
                $data= Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->where('order_status', '!=', '')->whereNotNull('order_status')->whereNotIn('order_status', ['Pending','Accepted'])->where('orders.branch_id',Auth::user()->branch_id)->select('orders.*', 'users.mobile_number', 'users.address')->orderBy('orders.id', 'desc')->paginate(1);
            }
            
            return view('branch.user.historypagination_list', compact('data'))->render();
     }
    }
    ...
    {
      return Excel::download(new BranchOrdersExport, 'Branchrollup.xlsx');
    }
}
